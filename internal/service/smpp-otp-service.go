...
import (
	...
	repository "gsm-otp-auth/internal/repository/interfaces"
...
func (s *OTPService) sendSMS(phoneNumber, otp string) error {
	...
	if err != nil {
		s.logger.ErrorLogger.Error(fmt.Sprintf("Failed to open serial port: %v", utils.Err(err)))
		return err
	}
	defer serialPort.Close()

	if _, err := serialPort.Write([]byte("AT\r")); err != nil {
		s.logger.ErrorLogger.Error(fmt.Sprintf("Failed to initialize modem: %v", utils.Err(err)))
		return err
	}
